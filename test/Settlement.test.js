...
const { WETH, CRSS } = require("./tokens");
...
describe("Settlement", function () {
    ...
    it("Should fillOrder()", async () => {
        ...
        const toToken = CRSS[chainId];

        // Set ratio of WETH:CRSS to 1:100
        await addLiquidity(
            users[0],
            fromToken,
            toToken,
        ...
        // Swap 10 CRSS with WETH manually to increase the price of CRSS
        await swap(users[0], await getTrade(toToken, fromToken, ethers.constants.WeiPerEther.mul(10)));

        const fromERC20 = await ethers.getContractAt("IPancakeERC20", fromToken.address);
        await helpers.expectToEqual(ethers.constants.Zero, fromERC20.balanceOf(users[1].address));

        // Calling fillOrder() now works because CRSS price increased more than 1%
    ...
    it("Should revert fillOrder() if called with a invalid signature", async () => {
        ...
        const fromERC20 = await ethers.getContractAt("IPancakeERC20", fromToken.address, users[1]);
