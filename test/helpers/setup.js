...
const { TokenAmount, Trade, Percent, Router } = require("@pancakeswap/sdk");
...
module.exports = async () => {
    ...
    const getTrade = async (fromToken, toToken, amountIn) => {
        const factory = await getContract("PancakeFactory");
    ...
    const swap = async (signer, trade, recipient = signer.address) => {
        const router = await getContract("PancakeRouter", signer);
        const fromER20 = await ethers.getContractAt("IPancakeERC20", trade.route.path[0].address, signer);
    ...
    const addLiquidity = async (signer, fromToken, toToken, fromAmount, toAmount, recipient = signer.address) => {
        const router = await getContract("PancakeRouter", signer);
        const fromER20 = await ethers.getContractAt("IPancakeERC20", fromToken.address, signer);
        await fromER20.approve(router.address, fromAmount);
        const toER20 = await ethers.getContractAt("IPancakeERC20", toToken.address, signer);
    ...
    const createOrder = async (signer, fromToken, toToken, amountIn, amountOutMin, deadline, overrides = {}) => {
        ...
        const fromERC20 = await ethers.getContractAt("IPancakeERC20", fromToken.address, signer);
